<?php
	...
	if (!$_REQUEST["username"] || !$_REQUEST["password"] || !$_REQUEST["new_username"]){
	...
	$new_username = "'".$conn->real_escape_string($_REQUEST["new_username"])."'";
	...
	$SQL = "UPDATE `employe_generique` SET `nom_utilisateur`=$new_username WHERE `nom_utilisateur`=$username";
	...
	echo '{"success":"Username changed successfully"}';
	$conn->close();
?>
